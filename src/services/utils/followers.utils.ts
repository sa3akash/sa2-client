...
export class FollowersUtils {
  static async followUser(user: any, dispatch: AppDispatch) {
  ...
  static async unFollowUser(user: any, profile: any, dispatch: AppDispatch) {
    const response = await followerService.unFollowUser(user?._id, profile?._id);
    Utils.addNotification(dispatch, { type: 'success', description: response.data?.message });
  }

  static async blockUser(user: any, dispatch: AppDispatch) {
    const response = await followerService.blockUser(user?._id);
    Utils.addNotification(dispatch, { type: 'success', description: response.data?.message });
  }

  static async unblockUser(user: any, dispatch: AppDispatch) {
    const response = await followerService.unblockUser(user?._id);
    Utils.addNotification(dispatch, { type: 'success', description: response.data?.message });
  }

  static socketIOFollowAndUnfollow(users: any[], followers: any[], setFollowers: any, setUsers: any) {
  ...
  static socketIORemoveFollowing(following: any[], setFollowing: (arg: any) => void) {
    socketService?.socket?.on('remove-follower', (data) => {
  ...
  static socketIOBlockAndUnblock(profile: any, token: any, setBlockedUsers: (arg: any) => void, dispatch: AppDispatch) {
    socketService?.socket?.on('blocked-user-id', (data) => {
      ...
      dispatch(addProfile({ token, profile: user }));
    });
  ...
  static socketIOBlockAndUnblockCard(user: any, setUser: (arg: any) => void) {
    socketService?.socket?.on('blocked-user-id', (data) => {
  ...
  static updateSingleUser(users: any[], userData: any, followerData: any, setUsers: (arg: any) => void) {
